...
function getPotentialPaths() {
  ...
  const vscodeExtDirs = [
    ...
    path.join(HOME, '.vscode-oss', 'extensions'),       // VSCodium
    ...
    try {
      if (fs.existsSync(extDir)) {
        ...
        for (const ext of extensions) {
          const binaryPath = path.join(extDir, ext, 'resources', 'native-binary', 'claude');
          if (fs.existsSync(binaryPath)) {
            paths.push({
              method: `VS Code extension (${ext})`,
              path: binaryPath,
              type: 'binary'
            });
          }
...
/**
 * Find ALL Claude Code installations (CLI + VS Code extensions)
 * @returns {Array<{ path: string, method: string, type: 'js' | 'binary' }>}
 */
