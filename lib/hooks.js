...
// @latest forces npx to re-resolve the dist-tag so new releases flow through
// (a bare package name would reuse a stale npx cache forever); -y skips the
// install prompt in non-interactive hook contexts
const HOOK_COMMAND = 'npx -y claude-depester@latest --silent --log --no-animation';
...
function installHook() {
  try {
    ...
    if (isHookInstalled()) {
      // Upgrade the command in place if it changed in a newer release
      let updated = false;
      for (const hookGroup of settings.hooks.SessionStart) {
        if (!hookGroup.hooks) continue;
        for (const hook of hookGroup.hooks) {
          if (hook.command && hook.command.includes('claude-depester') && hook.command !== HOOK_COMMAND) {
      ...
      if (updated) {
        ...
        return {
          success: true,
          message: `Hook command updated to: ${HOOK_COMMAND}`
        };
      }
      return {
        success: true,
